# REZET App Store Metadata (Science-Backed / Clinical Positioning)

## App Name (Max 30 characters)
REZET: Nervous System Reset

## Subtitle (Max 30 characters)
Physiological Anxiety Relief

*Alternative Subtitles:*
- Vagal Tone & State Regulation
- Somatic & Autonomic Regulation

## Keywords (Max 100 characters)
*Note: Do not use spaces after commas in App Store Connect to save characters.*
anxiety,panic,somatic,autonomic,vagus,nerve,hrv,respiratory,physiology,regulate,stress,nsdr,huberman

## Promotional Text (Max 170 characters)
Physiologically down-regulate your nervous system in 60 seconds. Use clinical respiratory pacing to shift from fight-or-flight back to autonomic homeostasis.

## What's New (Version 1.0.3)
- Create and save your own custom breathing protocols
- Set inhale, exhale, and optional hold timings
- Choose session targets by time or by rounds
- Start saved custom protocols from Home in one tap
- Stability fixes and session flow polish

## Full Description (Max 4000 characters)
What feels strongest right now? Is your heart racing? Is your chest tight? Are your thoughts looping?

REZET is a physiological toolkit, not a meditation app. It acts as an emergency brake for an overactive autonomic nervous system. 

When you experience acute stress, panic, or overwhelm, your biology is stuck in a sympathetic (fight-or-flight) or dorsal vagal (freeze) response. REZET provides you with the exact, science-backed respiratory protocols needed to mechanically stimulate your vagus nerve and force a physiological state shift back to baseline.

No spiritual fluff. Just clinical mechanics. Select your current somatic state, and follow the visual and haptic pacing.

NEW: MEASURE VITALS
Use the rear camera and flash for a dedicated 1-minute fingertip reading. A live camera preview helps you place your finger correctly, the measurement starts automatically once pulse lock is stable, and REZET processes the full capture window to estimate:

• HRV
• Heart rate
• Breathing rate

Breathing rate is inferred from cyclical heart rate changes during the measurement. Vitals are provided for wellness trends and self-awareness only, not diagnosis or medical monitoring.

NEW: CUSTOM BREATHING PROTOCOLS
Create and save your own breathing protocols with custom inhale, exhale, and optional hold timings. You can choose whether a session ends by total time or by completed rounds, then restart saved custom protocols in one tap from Home.

THE PROTOCOLS:
We map your acute symptoms to precise respiratory ratios designed to counteract them via heart rate variability (HRV) and vagal tone manipulation.

• Tachycardia (Heart Racing): 4-7-8 pacing directly stimulates the vagus nerve, utilizing breath retention to trigger a rapid parasympathetic response and lower heart rate.
• Chest Tension: 4-4-4-4 (Box Breathing) anchors the autonomic nervous system through equal respiratory distribution, preventing hyperventilation and releasing physical constraint.
• Racing Thoughts: Micro-varied interval pacing demands active cognitive tracking, disrupting Default Mode Network (DMN) loops.
• Sensory Overwhelm: Extended exhalations mechanically decrease heart rate via respiratory sinus arrhythmia (RSA), signaling biological safety to the brainstem.
• Dorsal Shutdown (Numbness): 6.0/6.0 coherent pacing gently up-regulates the system without triggering an adrenaline spike.
• Restlessness: 5.5s Resonant Frequency synchronization maximizes HRV, bringing the cardiovascular and respiratory systems into perfect alignment to clear cognitive fog.

SYSTEM FEATURES:
• Zero Friction: No long intro videos, no mandatory sign-ups. Immediate access to physiological regulation in two taps.
• Measure Vitals: A guided 1-minute camera reading for HRV, heart rate, and breathing rate estimates using the rear camera and flash.
• Custom Protocols: Create, save, and restart your own breathing sessions with custom timings and time-based or round-based targets.
• Clinical Insights: Detailed neurological and biological mechanics explaining why each protocol is effective.
• Haptic & Visual Pacing: A high-contrast adaptive interface combined with precise haptic feedback—allowing for screen-free, eyes-closed regulation.
• Adaptive Intensity Checkpoints: If a respiratory ratio exceeds your current CO2 tolerance, the algorithm safely down-scales the timing mid-session.
• Dose-Dependent Sessions: Choose 1, 3, or 5-minute intervals based on your immediate physiological requirement.

Stop trying to think your way out of a biological response. Use your physiology to change your psychology. Download REZET to gain mechanical control of your nervous system.

Disclaimer: REZET provides physiological wellness support and is not intended to be a substitute for professional medical advice, diagnosis, or treatment.
